...
@Data // not required
@NoArgsConstructor
@AllArgsConstructor // not required
...
Plusieurs classe de base utilise déjà la conversion manuelle, comme par exemple la classe Player,
Lorsque on set un joueur, il va juste mettre son pseudo ou son uuid dépendant du serveur et peut identifier le joueur avec le nom
Pour envoyer un joueur dans un message il suffit de faire
```java
    message.set("player",player);
```
Et pour lire le message il suffit de faire
```java
    Player player = message.get("player", Player.class);
```


Voici comment le joueur a été converti en message
```java
...
Et pour l'ajouter au système de conversion il suffit de faire
```java
     Message.getDefaultMapper().addMapper(new MapperOfPlayer());
```
